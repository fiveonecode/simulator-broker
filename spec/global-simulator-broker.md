...
| SB-SVC-RT-004 | Healthy status is HTTP `200`. `degraded` or `incompatible` status is HTTP `409`, uses reason `service-runtime-incompatible` and exit code `3`, and keeps `running: true` with exact service metadata so clients can withhold command authority while offering cooperative restart instead of treating the daemon as stopped or unverified. Other `409`, malformed, missing, mismatched, or transport-failed responses remain unverified. | `client/test/brokerd.test.mjs` status assertions; error-contract tests through `npm run test:client`; `app/Tests/BrokerSnapshotLoaderTests.swift`; `app/Tests/BrokerDashboardStoreTests.swift` |
...
- a current-generation app refresh failure preserves the last readable snapshot, tooling, and runtime paths but clears cached service authority until a later successful refresh validates exact service status; a live-status probe timeout, non-success HTTP response, or malformed response is a refresh failure rather than confirmed service absence, except that exact HTTP `409` status with reason `service-runtime-incompatible`, `running: true`, and matching selected service identity is verified live-but-unhealthy state that disables commands while retaining guided cooperative restart; a snapshot-only decode failure after confirmed service absence or verified restart-required status preserves recovery affordances, while the same failure after a validated healthy service remains unverified; the unverified state takes precedence even when no snapshot is cached and applies on first load when host configuration already exists; revocation, validated service-identity changes, and healthy-to-restart-required transitions dismiss pending mutation confirmations and invalidate delayed idle or lifecycle responses, revocation also cancels guided setup still previewing or awaiting confirmation, and an intentional setup cancellation refresh runs outside the cancelled task and publishes only while its setup generation still owns the flow so a late result cannot poison a replacement setup; service and local-setup confirmation paths reject revoked authority and must not present or offer recovery from unverified brokerd status; a true missing-host first load and a later failure after a cached missing-host success remain onboarding, a superseded failure must not downgrade newer loaded state, and setup already applying is not interrupted by refresh-driven dismissal; the unverified-status CLI fallback uses the same selected host-config, state-root, and service-socket paths as setup
...
| 0.16.4 | 2026-08-30 | Service identity changes invalidate pending mutations, confirmed absence survives snapshot corruption, and setup cancellation refreshes outside the cancelled task. |
